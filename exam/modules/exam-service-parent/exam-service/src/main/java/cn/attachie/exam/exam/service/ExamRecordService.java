...
import cn.attachie.exam.common.core.utils.DateUtils;
...
import cn.attachie.exam.common.core.utils.excel.ExcelToolUtil;
import cn.attachie.exam.common.core.vo.DeptVo;
...
import cn.attachie.exam.exam.api.dto.ExaminationRecordDto;
...
import cn.attachie.exam.exam.excel.model.ExamRecordExcelModel;
import cn.attachie.exam.exam.mapper.AnswerMapper;
import cn.attachie.exam.exam.mapper.ExamRecordMapper;
...
public class ExamRecordService extends CrudService<ExamRecordMapper, ExaminationRecord> {
    ...
    public void fillExamUserInfo(List<ExaminationRecordDto> examRecordDtoList, Long[] userIds) {
        ...
        ResponseBean<List<UserVo>> returnT = null;
                //userServiceClient.findUserById(userIds); todo:
        if (ResponseUtil.isSuccess(returnT)) {
            // 查询部门信息
            ResponseBean<List<DeptVo>> deptResponseBean =
            null;
                 // todo:   userServiceClient.findDeptById(returnT.getData().stream().map(UserVo::getDeptId).distinct().toArray(Long[]::new));
